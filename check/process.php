...
if($action=="read"){
	...
	$sql=$conn->query("SELECT * FROM users WHERE user='$login' or user=''");
	$users=array();
...
if($action=='create'){
	...
	$money=$_POST['money'];
	$user=$_POST['user'];
	$sql=$conn->query(" INSERT INTO users (location,day,money,user) VALUES ('$location','$day','$money','$login')");
...
{	$id=$_POST['id'];
	...
	if($sql){
		$result['message']="Ваши данные обновлены успешно!!";
	}
	else{
		$result['error']=true;
